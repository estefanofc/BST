...
- Part of printing implementation
  * warning: narrowing conversion from 'typename __gnu_cxx::__promote_2<int
, int>::__type' (aka 'double') to 'int'
         int MaxN = pow(2, Depth - 1);
- Avoided using pointers to reduce chances of memory leaks
  * non-const reference parameter 'Success', make it const or use a pointer
  * non-const reference parameter 'Index', make it const or use a pointer